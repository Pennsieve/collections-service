...
type FinalizeCollectionPublishFunc func(ctx context.Context, collectionID int64, userRole role.Role, request service.FinalizeDOICollectionPublishRequest) (service.FinalizeDOICollectionPublishResponse, error)
...
func (i *InternalDiscover) FinalizeCollectionPublish(ctx context.Context, collectionID int64, userRole role.Role, request service.FinalizeDOICollectionPublishRequest) (service.FinalizeDOICollectionPublishResponse, error) {
	if i.FinalizeCollectionPublishFunc == nil {
		panic("mock FinalizeCollectionPublish function not set")
	}
	return i.FinalizeCollectionPublishFunc(ctx, collectionID, userRole, request)
}
